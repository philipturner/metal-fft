...
extension MetalFFTTests {
    func testRandom1D(width: Int, isInverse: Bool, usingICB: Bool = false) throws {
        ...
        let fftInput1D = makeRandomFFTInput1D(width: width)
        let cpuOutput = desc.fft1D(fftInput1D)
        
        // Get GPU results
        
        let numElements = width
        let input = FFTComplexBuffer(device: device, capacity: numElements)
        fillFFTData(buffer: input, complexData: fftInput1D)
